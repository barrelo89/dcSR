...
//added from below
#include <libswscale/swscale.h> // Duin: for rgb->yuv and yuv-rgb conversion
#include "h264dec.h"

//Duin: 'stb_image' library to read rgb image into buffer or write the rgb images
...
//Duin: pgm_save (the same as the one in 'decode_video.c')
static void pgm_save(unsigned char *buf, int wrap, int xsize, int ysize,
 ...
 //Duin: ppm_save (the same as the one in 'decode_video.c')
  static void ppm_save(unsigned char *buf, int wrap, int ysize, char *filename)
  {
    FILE *f;
    int i;
...
//Duin: yuv->rgb conversion (the same as the one in 'decode_video.c') + save it as ppm file
static AVFrame* yuv420p_to_rgb24(uint8_t * buf[], int* wrap, int width, int height, AVFrame *rgb_frame, char* filename){
   int ret;

   //init once
   struct SwsContext *imgConvertCtxYUVToRGB = sws_getContext(width, height, AV_PIX_FMT_YUV420P, width, height, AV_PIX_FMT_RGB24, SWS_BILINEAR, NULL, NULL, NULL);

   av_image_alloc(rgb_frame->data, rgb_frame->linesize, width, height, AV_PIX_FMT_RGB24, 16);

   uint8_t *prgb24 = calloc(3 * width * height, sizeof(uint8_t));
   uint8_t *rgb24[1] = { prgb24 };
   int rgb24_stride[1] = { 3 * width };

   //ret = height of the converted video frame
   ret = sws_scale(imgConvertCtxYUVToRGB, buf, wrap, 0, height, rgb_frame->data, rgb24_stride);
...
//Duin: rgb->yuv conversion
//first load an image in rgb format using stb_image library (this libarary doesn't require any installation. You can just download it and place it in the directory)
 static void rgb24_to_yuv420p(uint8_t * data[], int *wrap, int width, int height, char* filename)
 {

     FILE *f = NULL;

     //we wait unitl 'filename' file (which is a SR image) is available
     while(!f){
         sleep(10);
         f = fopen(filename,"r");
     }
    ...
    //we read the SR image and keeps it in a buffer
    int channels = 0;
   ...
   //color format conversion: rgb->yuv to feed it back to ref_list
   //struct SwsContext * ctx = sws_getContext(width, height, AV_PIX_FMT_RGB24, width, height, AV_PIX_FMT_YUV420P, SWS_BILINEAR, NULL, NULL, NULL);
   struct SwsContext * ctx = sws_getContext(width, height, AV_PIX_FMT_RGB24, width, height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);

   int inLinesize[1] = { 3*width }; // RGB stride
   ret = sws_scale(ctx, &img, inLinesize, 0, height, data, wrap);
}

//Duin: this is the main place where the decoding process occurs
static int decode_simple_internal(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (HAVE_THREADS && avctx->active_thread_type & FF_THREAD_FRAME) {
        ...
        //printf("INSIDE THREAD DECODE Frame!\n");
    } else {
        //printf("OUTSIDE THREAD DECODE Frame!\n");
	...
	if ((frame->pict_type == AV_PICTURE_TYPE_I) && frame->width >= 1 && frame->height >= 1)
    ...
    //Duin: from this point, added code
        ...
        int sr = 0;
			{
        //https://ffmpeg.org/doxygen/0.6/structH264Context.html
        //I searched the size of how many ref_list we have
        //It seems like we have 2 ref_list and each is in length of 48

				for(int r = 0; r < 2; r++)
				{
					if (h->slice_ctx->ref_list[sr][r].data[0]) {
          //To Handle ref_list[0][0], ref_list[0][1]
			      char buf_before[1024], buf_after[1024];

						printf("sr : %d r : %d \n",sr,r);

						snprintf(buf_before, sizeof(buf_before), "%s-%d-%d.ppm", "decoded", sr,r);
						snprintf(buf_after, sizeof(buf_after), "%s-%d-%d.ppm", "sr", sr,r);
            ...
            //first convert the decoded pictures into rgb format and save it as a ppm file
						yuv420p_to_rgb24(
    								h->slice_ctx->ref_list[sr][r].data,
    								h->slice_ctx->ref_list[sr][r].linesize,
    								frame->width,
    								1080, //frame->height
    								rgb_frame,
    								buf_before);
            ...
            //rgb24_to_yuv420 waits until SR image is created and then convert it back into yuv format
						rgb24_to_yuv420p(
								h->slice_ctx->ref_list[sr][r].data,
								h->slice_ctx->ref_list[sr][r].linesize,
								frame->width,
            ...
            //printf("width : %d height : %d \n",frame->width,frame->height);
				    ...
				    //Make ref_list[0][0] = ref_list[1][1], ref_list[1][0] = ref_list[0][1]
            //Same image is used for writing the corresponding copy
						//printf("Making a copy of sr : %d r : %d to sr: %d r : %d\n",sr,r,1-sr,1-r);

            rgb24_to_yuv420p(
								h->slice_ctx->ref_list[1-sr][1-r].data,
								h->slice_ctx->ref_list[1-sr][1-r].linesize,
								frame->width,
								frame->height,
								buf_after);
					}
          else{
						printf(" data[0] buffer is NULL\n");
					}
				}
			}
		} else {
